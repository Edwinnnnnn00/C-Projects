...
static void MakeChange(int cost, int[] cashTill, int twenties, int tens = 0, int fives = 0, int ones = 0)
{
    ...
    if (changeNeeded > 0)
    {
        changeNeeded = amountPaid - cost - changeNeeded;

        while (changeNeeded > 19)
        {
            cashTill[3]++;
            changeNeeded -= 20;
            Console.WriteLine("\t Returning a twenty");
        }

        while (changeNeeded > 9)
        {
            cashTill[2]++;
            changeNeeded -= 10;
            Console.WriteLine("\t Returning a ten");
        }

        while (changeNeeded > 4)
        {
            cashTill[1]++;
            changeNeeded -= 5;
            Console.WriteLine("\t Returning a five");
        }
        ...
        while (changeNeeded > 0)
        {
            cashTill[0]++;
            changeNeeded -= 1;
            Console.WriteLine("\t Returning a one");
        }
